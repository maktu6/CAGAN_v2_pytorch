...
import os
import argparse
from utils import trainer, visualizer, cagan_dataset, options

opt = options.GatherOptions().parse()
if opt.mode == "train":
    train_dataset = cagan_dataset.CAGAN_Dataset(opt.data_root)
    train_dataloader = data.DataLoader(train_dataset, opt.batchsize, shuffle=True,
    ...
    if (opt.lr_policy == 'lambda') and (opt.niter_decay == -1):
        opt.niter_decay = opt.epoch*len(train_dataloader)*opt.batchsize - opt.niter
    model = trainer.CAGAN_Trainer(opt, save_dir=opt.save_dir)
    ...
    for epoch in range(opt.epoch):
        for real in train_dataloader:
            ...
            vis_custom.plot_current_images(model.get_current_visuals(), 'real-fake-rec-alpha')
    ...
    test_dataset = cagan_dataset.CAGAN_Dataset(opt.data_root, mode='test')
    test_dataloader = data.DataLoader(test_dataset, opt.batchsize, shuffle=False,
                                    num_workers=opt.num_workers, pin_memory=True)
    model = trainer.CAGAN_Trainer(opt, is_train=False)
    step = None
    if opt.step != -1:
        step = opt.step
